...
public class StudentUI {
    ...
    public void showStudentManagementMenu() {
        ...
        while (inLoop) {
            ...
            switch (choice) {
                case "1": handleDisplayStudentsWithPaginationAndSort(1); break;
                case "2": handleCreateStudent(); break;
                case "3": handleUpdateStudent(); break;
                case "4": handleDeleteStudent(); break;
                case "5": handleSearchStudentsWithPagination(); break;
                case "6":
                    int sortOpt = handleSelectSortOption();
                    if (sortOpt != 0) {
                        handleDisplayStudentsWithPaginationAndSort(sortOpt);
                    }
                    break;
    ...
    // PHÂN TRANG NÂNG CAO
    private int handleSelectSortOption() {
        ...
        try {
            ...
            System.out.println("[X] Lựa chọn không hợp lệ!");
        } catch (NumberFormatException e) {
            System.out.println("[X] Lựa chọn phải là ký tự số!");
        }
        return 0;
    }

    private void handleDisplayStudentsWithPaginationAndSort(int sortOption) {
        int currentPage = 1;
        int pageSize = 5;
        ...
        while (viewing) {
            try {
                int totalStudents = studentBusiness.countAll();
                int totalPages = (int) Math.ceil((double) totalStudents / pageSize);
                if (totalPages == 0) totalPages = 1;

                List<Student> pagedList = studentBusiness.findWithPaginationAndSort(currentPage, pageSize, sortOption);

                System.out.println("\n================ DANH SÁCH HỌC VIÊN (TRANG " + currentPage + "/" + totalPages + ") ================");
                printStudentTable(pagedList);

                System.out.println("\n--- [ĐIỀU HƯỚNG PHÂN TRANG HỌC VIÊN] ---");
                ...
                System.err.println("[LỖI HỆ THỐNG] Lỗi cơ sở dữ liệu: " + e.getMessage());
                viewing = false;
            }
        }
        pressEnterToContinue();
    ...
    private void handleSearchStudentsWithPagination() {
        System.out.println("\n============== TÌM KIẾM SINH VIÊN (PHÂN TRANG) ==============");
        System.out.print("Nhập từ khóa tìm kiếm (Tên/Email/ID): ");
        String keyword = scanner.nextLine().trim();

        if (keyword.isEmpty()) {
            System.out.println("[LỖI] Không được bỏ trống từ khóa tìm kiếm!");
            return;
        }
        ...
        while (viewing) {
            try {
                int totalStudents = studentBusiness.countSearch(keyword);
                int totalPages = (int) Math.ceil((double) totalStudents / pageSize);
                if (totalPages == 0) totalPages = 1;

                List<Student> pagedList = studentBusiness.searchWithPagination(keyword, currentPage, pageSize);

                System.out.println("\n================ KẾT QUẢ TÌM KIẾM CHO \"" + keyword + "\" (TRANG " + currentPage + "/" + totalPages + ") ================");
                printStudentTable(pagedList);

                System.out.println("\n--- [ĐIỀU HƯỚNG PHÂN TRANG TÌM KIẾM] ---");
                if (currentPage < totalPages) System.out.println("N. Trang tiếp theo (Next)");
                if (currentPage > 1)         System.out.println("P. Trang trước đó (Previous)");
                System.out.println("E. Thoát xem kết quả");
                System.out.print("Nhập hành động (N/P/E): ");
